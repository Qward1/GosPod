...
(function () {
  ...
  // Папка с реальными фотографиями наград (относительно корня статики).
  const PHOTO_BASE = "medals/";

  // Реальное фото награды (приоритет над SVG-стилизацией).
  function photo(src, alt) {
    return (
      `<img class="medal__photo" src="${PHOTO_BASE}${src}" alt="${_esc(alt)}" loading="lazy" ` +
      `decoding="async" />`
    );
  }
