...
class Store:
    ...
    def _init_db(self) -> None:
        with self._conn() as c:
            ...
            # updated_at нужен, чтобы кэш заметил правку «на месте»
            ...
            # профиль из вступительной анкеты; переиспользуется при повторных обращениях
    ...
    def update_ai_chat(
        ...
        *,
        ...
        # Закрепление не должно сдвигать чат по «последнему использованию».
    ...
    def add_usvo_card(self, card: dict, batch: str = "") -> int:
        """Сохраняет одну загруженную карточку УСВО."""
        now = time.time()
        with self._conn() as c:
            cur = c.execute(
                """
    ...
    def add_audit_log(
        ...
        """Пишет запись аудита; сбой аудита не должен ломать основное действие."""
        with self._conn() as c:
            cur = c.execute(
                """
