...
class MaxBot:
    ...
    async def _handle_message(self, message: dict) -> None:
        ...
        if chat_id != self.operator_chat_id:
            # активный сценарий мер: фото/текст направляем в него, а не в старый ЖКУ-флоу
            ...
            if flow_row:
                ...
                return
            # активный гид-опрос: ответы текстом и контакт отдаём ему
            ...
            if guided_row:
                ...
                return
            # файл, присланный вместо кнопки подтверждения, считаем самим заявлением
        ...
        # /start = начать сначала: сбрасываем профиль анкеты и текущий опрос
        ...
        # текстовый вход в сценарий мер (если фразу набрали вручную)
    ...
    async def _handle_user_question(
        ...
        # новичку сначала приветствие, затем обычный ответ на вопрос
        ...
        # фильтр темы на стороне LLM: отсекаем только уверенный off-topic;
        # при сомнении или сбое парсинга пропускаем, чтобы не отсечь живого человека
        ...
        if needs_operator:
            # телефон сохраняем в обращении — по нему ищется карточка УСВО
        ...
        # подбор меры по тексту: подходящую предлагаем кнопкой «Оформить»
    ...
    async def _submit_application(
        ...
        if not row:
            return
        # защита от повторной подачи по старым кнопкам
    ...
    async def _cancel_application(self, application_id: int) -> None:
        if not application_id:
            return
        row = self.store.get_application(application_id)
        # уже поданное заявление не понижаем
        if row and row["status"] == "submitted":
            return
        self.store.set_application_status(application_id, "rejected", "гражданин (отказ)")

    # --- Сценарий «Меры поддержки»: выбор меры → пошаговый сбор документов → распознавание → дозапрос полей → заявление → подтверждение → подача. ---
    ...
    # --- Гид-опрос «Меры поддержки»: тема → родство/регион → сводка профиля → телефон → выбор меры → (сбор документов) → регистрация + подписка. ---
    ...
    async def _ask_missing(
        ...
        # технические имена полей переводим на русский, прежде чем спрашивать гражданина
    ...
    async def _generate_and_offer(
        ...
        # подписи полей в превью — на русском
        labels = await self._ru_labels(measure)
    ...
    def _sla_business_days(self) -> int:
        """Срок ответа в днях: настройка из кабинета приоритетнее дефолта YAML."""
