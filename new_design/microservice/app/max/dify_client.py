...
# правила сегментации базы знаний: DEFAULT — чанки до 4000 токенов по \n\n,
# SINGLE_CHUNK — документ одним чанком (для карточек УСВО); pre_processing выключен,
# чтобы не резались ссылки и форматирование
...
class DifyClient:
    ...
    async def ask(self, query: str, user_key: str) -> dict:
        """Запрос к MAX-ветке Dify; при любом сбое отдаёт безопасный дефолт (вопрос уйдёт оператору)."""
        ...
        # лог для диагностики: сырой ответ ветки и результат разбора
        log.info(
    ...
    async def upsert_document_text(
        ...
        if doc_id:
            ...
            if not _dify_failed(result):
                return result
            # старый документ недоступен для обновления — пересоздаём заново
            log.warning(
...
def _parse_structured(raw: str) -> dict:
    ...
    if not isinstance(parsed, dict):
        # не распарсили — уходит на эскалацию, тему считаем релевантной
        return {
