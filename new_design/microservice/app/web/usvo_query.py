...
# бюджет символов на детерминированный блок, держим ниже окна модели
...
def plan_offline(question: str, meta: UsvoCardsMetaService | None = None) -> QuerySpec:
    ...
    # статусы матчим по основе слова, чтобы ловить склонения
    if meta is not None:
        try:
    ...
    # «кто/у кого …» с фильтрами — перечисление; информационные вопросы остаются semantic
...
def _spec_from_dict(data: dict) -> QuerySpec:
    ...
    for key in ("age_min", "age_max", "limit"):
        val = data.get(key)
        # 0/отрицательное от планировщика считаем «не задано»
...
# --- Новый конвейер: вопрос -> JSON-фильтр -> валидация -> SQLite-индекс ---
# в отличие от плоского QuerySpec умеет вложенные AND/OR и любые поля карточки

# инструкция для планировщика; заодно описывает сам DSL
