...
// каноничный список полей: дедуп по названию + гарантированные поля шапки;
// на сохранение уходит полный набор, поэтому ничего не теряется
...
function groupFields(r) {
  ...
  // «Дополнительно» показываем всегда — сюда падают несгруппированные и новые поля
...
/* ---------- offcanvas: карточка выезжает справа, список остаётся слева ---------- */
...
/* Связь обращения с карточкой УСВО по телефону: один матч — ссылка,
   несколько — оператор выбирает из списка. */
function usvoLinkBlock(a) {
  ...
  if (a.usvo_id && matches.length === 1) {
    // показываем, по чему определена связь: телефон — надёжно, ФИО — лишь
    // предположение, его оператор должен проверить
...
function openAppeal(id) {
  ...
  if (!a) return;
  // сотрудник отвечает только от своего имени, админ выбирает любого
...
/* Итог импорта: сколько загружено, сколько пропущено как дубли и почему. */
...
async function selectUsvo(id) {
  ...
  // поле правится на месте (contenteditable); при сохранении набор собирается
  // из DOM целиком, скрытые поля не теряются
...
/* ---------- инлайн-редактирование карточки УСВО ----------
   поля правятся на месте; ключевые данные пересчитывает бэкенд */
function bindUsvoInlineEdit(r, id) {
  // слушатели вешаем на свежесозданные узлы, иначе они копились бы
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    // history_raw шлём, только если текст истории реально меняли
...
function renderAiText(text) {
  ...
  // markdown-таблицы рендерим настоящими таблицами, остальное экранируем
...
/* === РАЗДЕЛ 4 — АНАЛИТИКА === */
/* ---------- загрузка в базу знаний (тот же датасет, что у бота) ---------- */
...
async function renderAnalytics() {
  ...
  // бары рендерятся пустыми и плавно заполняются после отрисовки (animateCharts)
...
/* ----- график «Динамика по показателям»: переключение кнопками, с анимацией ----- */
...
function drawDyn() {
  ...
  // анимация прорисовки: гоним dashoffset к 0; длину умножаем на масштаб,
  // т.к. штрих считается в экранных пикселях (non-scaling-stroke)
...
/* Тепловая карта: Leaflet инициализируется после вставки разметки в DOM. */
...
/* Регламент ответа (SLA): правится в кабинете, хранится в БД,
   приоритетнее дефолта из конфига. */
  ...
  return `
    ...
    <div class="bc-targets">
      <label class="bc-target"><input type="radio" name="bc-target" value="all" checked />
        <span><b>Все пользователи MAX</b><small>${total} чел. — все, кто когда-либо писал боту</small></span></label>
      <label class="bc-target"><input type="radio" name="bc-target" value="subscribers" />
        <span><b>Только подписчики</b><small>${subs} чел. — подписаны на уведомления</small></span></label>
    </div>
...
async function initUserBox() {
  try {
    ...
    // сотрудник отвечает от своего имени, админ может выбрать любое
...
async function init() {
  ...
  // OAuth-callback приходит в hash — не затираем его и не считаем маршрутом SPA
