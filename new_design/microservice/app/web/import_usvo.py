...
# рекомендуемые столбцы для шаблона; импорт принимает любые заголовки
TEMPLATE_COLUMNS = [
    "ФИО УСВО",
    "Дата рождения",
    "Статус",
...
# примеры строк; вторая — со свободной историей для показа нормализатора
TEMPLATE_ROWS = [
    [
...
def parse_usvo_xlsx(file_bytes: bytes) -> list[dict]:
    ...
    for row in rows:
        ...
        # карточку без имени принимаем, полностью пустую — нет
