...
# временный режим full_context: вся база карточек подаётся модели напрямую
...
# формат таблицы повторяет шаблон справки, показатели модель подбирает по вопросу
...
class AiChatService:
    def __init__(
        ...
        *,
        ...
        # необязательный Dify-планировщик; без него вопрос разбирается офлайн
        self.planner = planner
        # SQLite-индекс карточек; без него движок сканирует карточки в Python
        self.index = index
    ...
    async def send_message(
        ...
        if self.full_context_enabled():
            # временный режим: вся база в контексте, модель считает сама
            ...
            # точный подсчёт по всем карточкам, минуя лимит контекста модели
            try:
