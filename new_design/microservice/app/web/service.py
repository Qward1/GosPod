...
SEED_ANSWERS = [
    "Статус ветерана боевых действий оформляется через военный комиссариат или МФЦ. "
    "Понадобятся паспорт, военный билет и документы, подтверждающие участие в СВО. "
    "После рассмотрения выдаётся удостоверение ВБД — по нему открываются льготы и выплаты.",
    "После ранения доступны единовременная выплата, ежемесячная денежная компенсация "
    "и оплата лечения/реабилитации. Размер зависит от группы инвалидности и заключения ВВК. "
    "Подайте заявление в соцзащиту или через Госуслуги — поможем собрать пакет документов.",
    "Да, компенсация санаторно-курортного лечения положена участникам СВО и ветеранам БД "
    "при наличии медицинских показаний. Обратитесь в соцзащиту Ленинского г.о. с направлением "
    "и справкой формы 070/у — подскажем актуальные сроки подачи.",
    "Помощь в трудоустройстве оказывает служба занятости и программа «Время героя». "
    "Можно записаться на консультацию, подобрать вакансии и пройти собеседование. "
    "Оставьте удобное время — передадим заявку специалисту по трудоустройству.",
    "Да, бесплатное переобучение доступно через службу занятости и партнёрские центры. "
    "Направления: IT, логистика, охрана, рабочие специальности. Запись — по паспорту "
    "и статусу участника СВО; места открываются по мере набора групп.",
    "Участникам СВО и членам семей положена компенсация расходов на ЖКУ "
    "(размер зависит от региональных нормативов). Оформление — в МФЦ или через Госуслуги "
    "с документами на жильё и удостоверением/справкой участника.",
    "Психологическая помощь доступна бесплатно: очно в центре поддержки и дистанционно. "
    "Запись по телефону горячей линии или через оператора кабинета. "
    "Можем поставить вас на ближайшее свободное окно на этой неделе.",
    "Да, матери погибшего положена пенсия по потере кормильца при соблюдении условий СФР. "
    "Понадобятся свидетельство о смерти, документы о родстве и справка о праве на пенсию. "
    "Поможем подготовить заявление и записаться на приём в Социальный фонд.",
    "Технические средства реабилитации (ТСР) выдаются по ИПРА. "
    "Сначала нужно заключение МСЭ, затем заявление в СФР или через Госуслуги. "
    "Перечень положенных изделий зависит от группы инвалидности — проверим ваш комплект документов.",
    "Для детей участников СВО доступны выплаты, льготы на питание/лагеря и внеочередное "
    "зачисление в детский сад и школу. Конкретный набор мер зависит от возраста ребёнка "
    "и статуса родителя — уточните данные, подберём актуальный перечень.",
    "Юридическая консультация по жилищным вопросам проводится бесплатно для участников СВО "
    "и членов семей. Можно прийти на приём или получить письменную справку. "
    "Опишите ситуацию кратко — передадим юристу и вернёмся с ответом.",
    "Вступление в ассоциацию ветеранов СВО — через заявление и копии документов участника. "
    "После рассмотрения выдаётся членский билет, открывается доступ к мероприятиям и мерам поддержки. "
    "Форму заявления можно получить у нас или скачать на сайте ассоциации.",
]
...
class WebService:
    ...
    def _audit(self, action: str, *, actor: dict | None = None, entity: str = "",
        ...
        """Запись в аудит-лог; ошибка аудита не ломает основное действие."""
        try:
    ...
    def _build_seed_appeals(self) -> list[dict]:
        ...
        for i, rec in enumerate(sample):
            ...
            out.append({
                ...
                "answer": SEED_ANSWERS[i % len(SEED_ANSWERS)] if answered else "",
    ...
    def _escalation_phone(self, d: dict) -> str:
        """Телефон гражданина: из обращения либо из профиля MAX."""
    ...
    def list_appeals(self) -> list[dict]:
        ...
        for a in appeals:
            ...
            # лёгкие инсайты для таблицы: тональность и «суть кратко», без LLM
    ...
    async def draft(self, appeal_id: str, operator: str) -> dict:
        ...
        # в шапке ответа — имя гражданина из MAX, карточка даёт только контекст
    ...
    def usvo_statuses(self) -> list[str]:
        """Статусы участников СВО для фильтра."""
        from app.web.usvo import STATUSES
    ...
    async def import_usvo(self, file_bytes: bytes, replace: bool = False,
        ...
        # совпадающие с базой записи пропускаем: правленая версия остаётся, дубль не плодится
        ...
        saved_ids: list[int] = []
        skipped = 0
        # подсказки оператору: что пропущено как дубль и почему
    ...
    def _application_to_dict(self, row) -> dict:
        ...
        usvo = self._match_usvo(
            d.get("user_name") or applicant.get("fio") or "",
            applicant.get("phone") or "",
        )
        # Подставляем пустые поля заявителя из карточки УСВО / имени в заявке.
        if not applicant.get("fio"):
            applicant["fio"] = (d.get("user_name") or "") or (usvo.name if usvo else "")
        if usvo:
            if not applicant.get("birth_date"):
                applicant["birth_date"] = usvo.birth_date or ""
            if not applicant.get("phone"):
                applicant["phone"] = usvo.phone or ""
            if not applicant.get("address"):
                applicant["address"] = usvo.address or ""
        result = {
            "id": d["id"],
            ...
            "citizen": {
                "name": d.get("user_name") or applicant.get("fio") or "",
                "username": d.get("username") or "",
            },
            "applicant": applicant,
            ...
            "user_files": list(raw.get("user_files") or []),
        }
        ...
        if d.get("support_measure_id") or "fields" in raw:
            ...
            if raw.get("documents"):
                result["documents"] = list(raw.get("documents") or [])
            result["summary"] = "\n".join(summ)
        return result
    ...
    def analytics(self, series_days: int = 30, series_end=None) -> dict:
    ...
    # стабильное имя документа с id — синхронизация обновляет его без дублей
    @staticmethod
...
def _dedup_reason(ident: str) -> str:
    """Причина, по которой запись посчитали дублем."""
...
def _tri(want, actual: bool) -> bool:
    """Трёхпозиционный фильтр: True — запись надо отсеять."""
