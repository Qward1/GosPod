...
# все эндпоинты кабинета под сессией; логин-роуты вынесены отдельно
...
# должно идти до маршрута с {rec_id:int}, иначе "template" попадёт в него
...
async def analytics(
    ...
    if end:
        try:
            ...
            raise HTTPException(status_code=400, detail="Некорректная дата end (YYYY-MM-DD)") from exc
