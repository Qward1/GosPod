...
# --- Демо-базис дашборда: реальные числа суммируются со стабильным базисом ---
# тематики обращений (базис)
FAKE_TOPICS = {
...
# тепловая карта: очаги с реальными координатами посёлков Ленинского ГО
...
def _aux_daily(seed: int, weekday_base: list[int], days: int = 14, end: dt.date | None = None) -> list[int]:
...
def _hourly_weights() -> list[int]:
    return [1, 1, 1, 1, 2, 3, 5, 8, 12, 14, 13, 12, 11, 12, 13, 12, 10, 8, 6, 4, 3, 2, 2, 1]
...
def _distribute_hourly(
    ...
    """Почасовой ряд за указанные сутки (подписи HH:00)."""
    day = day or msk_today()
    weights = _hourly_weights()
    wsum = sum(weights) or 1
    base = [max(0, int(round(day_total * w / wsum))) for w in weights]
    diff = day_total - sum(base)
    i = 0
    while diff != 0 and base:
        idx = (seed + i) % 24
        if diff > 0:
            base[idx] += 1
            diff -= 1
...
def _build_series(
    ...
    real = _trend(appeal_times, days, end=end)
    for k in range(days):
    ...
    return [
        {
            "key": "day", "label": "Обращений за день", "unit": "обр./день", "unit_hourly": "обр./час",
            "points": _series_points(dates, day_vals),
            "hourly": _distribute_hourly(day_vals[-1], 401, appeal_times, day=end),
        },
        {
            "key": "week", "label": "Обращений за неделю", "unit": "обр./7 дн.", "unit_hourly": "обр./час",
            "points": _series_points(dates, week_vals),
            "hourly": _distribute_hourly(max(1, week_vals[-1] // 7), 503, None, day=end),
        },
        {
            "key": "month", "label": "Обращений за месяц", "unit": "обр./30 дн.", "unit_hourly": "обр./час",
