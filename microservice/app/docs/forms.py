...
Заявление — реальная форма Минсоцразвития Московской области:
«Заявление о назначении ежемесячной денежной компенсации расходов по оплате
жилого помещения и коммунальных услуг отдельным категориям граждан»
(исходник — Zayavlenie_o_naznachenii.docx).

Здесь:
  • DEFAULT_MEASURE — мера поддержки, под которую оформляется заявление;
  • normalize_application — приведение «сырых» данных (из визуальной модели или
    фолбэка) к единой структуре с дефолтами;
  • build_application_docx — рендер заполненного заявления в Word (через
    app.common.docx, без сторонних зависимостей);
  • application_summary — короткий человекочитаемый текст для MAX-сообщения и
    карточки в админ-разделе «Заявления».

Те же плейсхолдеры используются Dify-ассистентом document_assistant.yml — список
обязан совпадать (см. PROMPTS.md).
"""
