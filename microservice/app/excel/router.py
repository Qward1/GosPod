...
Файл можно передать тремя способами (микросервис принимает любой — это снимает
зависимость от версии Dify и формата HTTP-узла):
  1. multipart/form-data, поле `file`        — основной способ;
  2. поле/JSON `file_url` (или ?file_url=)    — микросервис скачает файл сам;
  3. сырой бинарный body (application/octet-stream).
"""
...
async def extract(
    ...
    Ответ:
        {
          "text": "...",            # все записи таблицы, пустые ячейки убраны
          "rows": 50,               # сколько записей включено в текст
          "total_rows": 50,         # сколько записей с данными в таблице
          "columns": [...],         # заголовки столбцов
          "char_count": 43908,
          "token_estimate": 21954,  # грубая оценка (~2 символа кириллицы на токен)
          "truncated": false        # true, если данные пришлось урезать под лимит 32k
        }
    """
