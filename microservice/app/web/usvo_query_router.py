...
`POST /api/web/usvo/query` закрывает шаг «HTTP Request → backend» из схемы: принимает либо
готовый JSON-фильтр, либо вопрос (тогда фильтр строит планировщик — Dify или офлайн-эвристика),
валидирует его и исполняет в SQLite-индексе (app/web/usvo_index.py). Возвращает количество,
агрегаты и компактный список карточек. Используется и для тестирования пайплайна, и как точка
входа для Dify-графа.

`GET /api/web/usvo/query/catalog` — справочник полей/операторов (для построения фильтров и отладки).
"""
