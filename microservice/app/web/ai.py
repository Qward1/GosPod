"""ИИ-функции веб-кабинета: черновик ответа гражданину и предложения по мерам
поддержки.

Ответы могут браться из отдельного Dify-приложения для веба или из любой
OpenAI-compatible модели. Если внешний ИИ не настроен или недоступен, функции
корректно деградируют до офлайн-шаблонов, чтобы кабинет оставался рабочим.
"""
...
def _tokens(text: str) -> set[str]:
    """Грубая токенизация для оценки пересечений: слова от 5 букв, усечённые до
    6 символов (примитивный стемминг, чтобы «выплаты»/«выплата» совпадали)."""
...
def _draft_confidence(
    *, source: str, usvo: UsvoRecord | None, question: str, kb_text: str, body: str
    ...
    Это не вероятность модели, а прозрачный индикатор полноты исходных данных —
    оператору видно, насколько черновик опирается на реальный контекст. В отличие
    от прежней версии (несколько булевых факторов → почти всегда одно и то же число)
    оценка зависит от КОНКРЕТНОГО обращения и потому реально меняется:

      • q_in_answer — насколько черновик покрывает термины вопроса;
      • q_in_kb     — насколько вопрос освещён в базе знаний (есть ли о чём отвечать);
      • profile     — полнота сопоставленной карточки УСВО;
      • depth       — развёрнутость ответа.
    """
    q = _tokens(question)
...
def _draft_user_prompt(cfg: Config, question: str, usvo: UsvoRecord | None) -> str:
    ...
    Системный промт (роль, формат, ограничения) задаётся в самом Dify-приложении
    — см. ai_prompts.txt. Здесь собирается только динамическая часть запроса.
    """
    kb = _kb_excerpt(cfg)
    kb_part = f"\n\nБаза знаний:\n{kb}" if kb else ""
    return (
...
async def _ask_external_ai(cfg: Config, prompt: str, user_key: str) -> tuple[str, str]:
    ...
    if provider == "dify" and _configured(ai.dify_app_key):
        # Запрос уходит во входную переменную Dify `query`; системный промт —
        # внутри самого Dify-приложения (см. ai_prompts.txt).
...
async def draft_reply(
    ...
    *,
    ...
    Шапка с ФИО подставляется всегда (известные данные), тело — из внешнего ИИ
    либо подсказка-заглушка, если ИИ не настроен.

    Имя в обращении берётся из РЕАЛЬНОГО профиля гражданина MAX (`citizen_name`),
    а не из случайно сопоставленной карточки УСВО. Карточка используется только как
    контекст для тела ответа.
    """
...
def _local_suggestions(usvo: UsvoRecord) -> list[dict]:
    ...
    Каждый пункт — с приоритетом, конкретным следующим шагом, перечнем документов
    и местом обращения, чтобы оператор мог действовать без доуточнения.
    """
    items: list[dict] = []
    flags = usvo.flags
...
async def suggest_measures(
    ...
    *,
    ...
    Базовый список считается по правилам из полей карточки (всегда). Если Dify
    настроен — добавляем обоснованную базой знаний заметку ИИ.
    """
