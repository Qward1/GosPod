/* Реестр наград → изображения медалей для карточки УСВО.
   Для наград, у которых есть реальное фото (папка static/medals/), отдаётся <img>
   с фотографией государственной награды; для остальных — узнаваемая стилизованная
   SVG-медаль, рисуемая программно (офлайн-фолбэк, не точная копия).
...
(function () {
  ...
  // База статики кабинета. Карточка УСВО открывается по deep-link
  // `/usvo/cards/<id>` (history.pushState), поэтому относительный путь
  // `medals/...` резолвится в `/usvo/cards/medals/...` → 404. Считаем
  // абсолютный префикс из текущего pathname так же, как app.js::appBaseFromPath.
  function _photoBase() {
