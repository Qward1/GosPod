...
from app.web import usvo as usvo_mod
...
# Ответы к демо-обращениям — по одному на вопрос, ТОТ ЖЕ ПОРЯДОК, что в
# SEED_QUESTIONS. Раньше во всех отвеченных обращениях стояла заглушка «Ответ
# направлен гражданину», из-за чего панель ответа и выгрузка в Excel выглядели
# пустыми. Это демо-данные, а не справочник: реальные ответы пишут операторы.
SEED_ANSWERS = [
...
class WebService:
    ...
    def usvo_statuses(self) -> list[str]:
        """Статусы участника СВО для выпадающего фильтра.

        Сначала канонический справочник (`usvo.STATUSES`) — фиксированный состав
        и порядок, под них же настроена подсветка бейджей во фронтенде. Затем
        добавляем нестандартные значения, реально встретившиеся в данных (чужая
        выгрузка со своим написанием), чтобы такие карточки оставались доступны
        из фильтра, а не только текстовым поиском.
        """
        out = list(usvo_mod.STATUSES)
        seen = {s.lower() for s in out}
        for r in self._all_records():
            s = (r.status or "").strip()
            if s and s not in ("—", "-") and s.lower() not in seen:
                seen.add(s.lower())
                out.append(s)
        return out
    ...
    def analytics(self, series_days: int = 30, series_end: "dt.date | None" = None) -> dict:
        """Сводка дашборда. `series_days`/`series_end` задают окно графика
        «Динамика по показателям» (день/неделя/месяц с выбором периода)."""
        ...
        return build_analytics(
            ...
            series_days=series_days, series_end=series_end,
        )
