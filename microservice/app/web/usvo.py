...
Источник — тот же Excel, что и в задании 1 (`USVO_*.xlsx`), путь задаётся в
`web.usvo_xlsx`. Колонки раскладываются на ПЕРВИЧНЫЕ и ВТОРИЧНЫЕ данные ровно так,
как описано в макете (раздел «Персональная карточка УСВО»).

Файл кэшируется и перечитывается автоматически, если изменился (по mtime), —
таблицу можно править без перезапуска сервиса.
"""
...
# Индексы столбцов (1-based) по макету. Раскладка соответствует таблице
# USVO_*.xlsx (34 столбца). Если таблица другой структуры — недостающие индексы
# просто пропускаются, лишние уходят в группу «Дополнительно».
...
# Поручения Главы округа: личные встречи и тематические поручения. Назначаются
# части карточек детерминированно (по id), чтобы строка «поручение Главы» была
# не у всех, а лишь над некоторыми карточками — как в задании.
...
def _maybe_excel_date(value: str) -> str:
    ...
    В xlsx дата иногда хранится числом с форматом без признака даты (General),
    и _fmt не распознаёт её как дату — на выходе остаётся голое число вроде «27960».
    Для столбцов, которые заведомо содержат дату, доводим такое число до даты.
    Диапазон 10000..51000 ≈ 1927..2039 — отсекает счётчики и телефоны.
    """
    s = (value or "").strip()
    if not s.isdigit():
        return value
...
def _synthetic_value(header: str, seq: int) -> str:
    ...
    if "дата рождения" in h:
        # Реалистичная дата рождения совершеннолетнего участника СВО: год 1972–2003
        # (≈23–54 года на 2026-й), с разнообразными днём и месяцем. Детерминировано
        # по seq, но без «сползания» в будущее, как было раньше.
...
# ============================================================================
# Построение карточки из произвольного набора полей (label → value).
# Используется загрузчиком карточек из веб-кабинета (usvo_db.py): таблица,
# которую загрузил оператор, может иметь любой набор столбцов, поэтому первичные/
# вторичные данные и флаги для аналитики определяются по СМЫСЛУ заголовка, а не по
# фиксированной позиции столбца (как в каноничной таблице из задания 1).
# ============================================================================

def _label_matches_key(label: str, key: str) -> bool:
    ll = (label or "").lower()
    # Загруженные таблицы часто имеют простой заголовок «Статус» (без хвоста, который
    # ловит трюк с пробелом в KEY_HEADERS для каноничной таблицы). Поэтому для пути
    # загрузки сопоставляем «статус» как подстроку, исключая «примечание к статусу».
    if key == "status":
...
def find_field_value(fields: list[tuple[str, str]], key: str) -> str:
    ...
    Используется ТОЛЬКО для карточек, загруженных из веб-кабинета (произвольный
    набор столбцов). Каноничная Excel-таблица из задания 1 разбирается отдельно
    через UsvoStore._key_index и этот матчинг не затрагивает.
    """
...
def record_from_fields(
    ...
    *,
    ...
    Пустые значения отбрасываются. Все непустые поля попадают в `primary`
    (фронтенд всё равно перегруппировывает их по смыслу), ключевые значения
    (ФИО, статус, телефон…) дополнительно выносятся в шапку.
    """
...
#
# Один и тот же участник должен опознаваться, даже если часть данных в карточке
# изменили вручную (телефон, адрес, статус, награды и т. п.). Поэтому ключ строится
# из НАИБОЛЕЕ устойчивых полей — ФИО + дата рождения; при отсутствии даты —
# ФИО + телефон; в крайнем случае — только ФИО или только телефон. Редактирование
# второстепенных полей идентичность не меняет, поэтому при загрузке Excel с уже
# изменёнными карточками дубликаты отсеиваются, а в базе остаётся изменённая версия.
# ============================================================================
